...
@rem Where log mapreduce.files are stored.  %HADOOP_HOME%/logs by default.
@rem set HADOOP_LOG_DIR=%HADOOP_LOG_DIR%\%USERNAME%

@rem Where log mapreduce.files are stored in the secure data environment.
...
@rem The directory where pid mapreduce.files are stored. /tmp by default.
